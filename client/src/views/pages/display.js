...
const toastrConfirmOptions = {
  onOk: () =>
    $.ajax({
      method: 'delete',
      crossDomain: true,
      url: config.BACKEND_URL + "/v1/users/roommates/" + localStorage.getItem("session_userid"),
    }).done((res) => {
      console.log(res);
...
class Display extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      fetch(config.BACKEND_GSURVEY + "/api/v2/admin/surveys/"+this.props.location.state.surveyid, options)
      if (!response.ok) {
